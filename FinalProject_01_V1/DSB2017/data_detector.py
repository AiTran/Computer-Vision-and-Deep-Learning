...
class DataBowl3Detector(Dataset):
    def __init__(self, split, config, phase = 'train',split_comber=None):
        ...
        print(data_dir)
        data_dir_image = data_dir 
        data_dir_label = data_dir.replace("_img","_label").replace(" ","")
        print(data_dir_label)
        self.pad_value = config['pad_value']
#        print(data_dir)
#        print(data_dir_image)
#        print(data_dir_label)
        self.split_comber = split_comber
#        idcs = split
        idcs = os.listdir(data_dir_label)
        if phase!='test':
            idcs = [f for f in idcs if f not in self.blacklist]

        self.channel = config['chanel']
        if self.channel==2:
            self.filenames = [os.path.join(data_dir_image, '%s_merge.npy' % idx) for idx in idcs]
        elif self.channel ==1:
            if 'cleanimg' in config and  config['cleanimg']:
                self.filenames = [os.path.join(data_dir_image, '%s_clean.npy' % idx.replace("_label.npy","")) for idx in idcs]
            else:
                self.filenames = [os.path.join(data_dir_image, '%s_img.npy' % idx) for idx in idcs]
        ...
        # for idx in idcs:
        #     print(idx)
        #     idx = idx.replace("_label.npy","").replace(" ","")
        #     if config['luna_raw'] ==True:
        #         try:
        #             l = np.load(os.path.join(data_dir_label.replace(" ",""), '%s_label_raw.npy' %idx))
        #         except:
        #             l = np.load(os.path.join(data_dir_label.replace(" ",""), '%s_label.npy' %idx))
        #     else:
        #         l = np.load(os.path.join(data_dir_label.replace(" ",""), '%s_label.npy' %idx))
        l = np.load(data_dir_label + "/" + os.listdir(data_dir_label)[0])
        labels.append(l)
        print(1111111111111111)
        print(data_dir_label + "/" + os.listdir(data_dir_label)[0])
	...
	#	label[label==-1]=0
            ...
            [a, imgs] = np.load(self.filenames[idx], allow_pickle=True)
            print("======================", imgs.shape)
            ...
            # print(bboxes, torch.from_numpy(coord2.astype(np.float32)), np.array(nzhw))
...
#         flipid = np.array([np.random.randint(2),np.random.randint(2),np.random.randint(2)])*2-1
    ...
    print("------agument----")
    # print(sample, target, bboxes, coord )
...
class Crop(object):
    ...
    def __call__(self, imgs, target, bboxes,isScale=False,isRand=False):
        ...
        print("-----crop-------\n")
        # print(crop, target, bboxes, coord)
