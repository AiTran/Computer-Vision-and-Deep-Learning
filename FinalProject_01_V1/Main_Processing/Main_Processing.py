...
class Process_Image(Ui_Form):
    ...
    def onCurrentIndexChanged(self, indeximg):
        #print("combobox changed", indeximg)
        # print(self.Path_Original)
        # Name_Image = self.List_Image[indeximg]
        self.Path_img = self.input_dir + "/" + str(self.comboX.currentText())
        
        self.Load_numpy_img = np.load(self.Path_img, allow_pickle=True)
        
        # cv2.imwrite(Path_img.replace(".npy",".jpg"),Load_numpy_img[0,129])
        # _,height, width = self.Load_numpy_img.shape
        # bytesPerLine = 1 * width
        # qImg = QImage(self.Load_numpy_img[0,0].data, width, height, QImage.Format_RGB16)
        # img = QPixmap(qImg) 
        ...
        self.label_Input.setGeometry(20, 170, 500, 500)
...
##############################################################################
    ...
    #def Choose_Slice_Original(self):
        
        
        # self.Slider_original.valueChanged.connect(self.Change_Value_Original)
        # self.Slider_original.setMinimum(0)
        # self.Slider_original.setMaximum(self.Load_numpy_img.shape[1]  - 1)
    
    def Change_Value_Original(self):
        size = self.Slider_original.value()
        self.Load_numpy_img = np.load(self.Path_img)
        
        self.Load_numpy_img = np.expand_dims(self.Load_numpy_img, axis=0)
        #print(self.Load_numpy_img.shape)
        cv2.imwrite(self.Path_img.replace(".npy",".jpg"),self.Load_numpy_img[0,size])
        img = QPixmap(self.Path_img.replace(".npy",".jpg") )
        self.label_Input.setPixmap(img)
        # self.label_Input.setGeometry(20, 120, 500, 400)
        self.label_Input.show()
...
########################################################
    def Showimg_preprocessing(self):
      
        # self.Path_Original = str(self.comboX.currentText())
        # # print(self.Path_Original)
        # self.im = QPixmap(self.input_dir +"/" + self.Path_Original ) 
        # # self.Input_label = QLabel()
        # self.label_Input.setPixmap(self.im) 
        # self.label_Input.setGeometry(20, 110, 411, 331) 
        # self.label_Input.show()
        # self.folderimg = self.input_dir
        # self.newfodler  = self.folderimg.split("/")[0:-1]
        # print(self.newfodler)
        # joinfolder= "/".join(self.newfodler)
        # print(joinfolder)
        # # str1= Image_Pr
        print("Minh nmap ngu ngu")
    ...
    def changeValue(self):
        ...
        #cv2.imwrite("./Image/Image_Pr/001_clean.jpg",img[0,size])
        #self.im = QPixmap('./Image/Image_Pr/001_clean.jpg' ) 
        # self.label_Pre_Processing.setPixmap(self.im)
        self.label_Pre_Processing.setPixmap(self.im)
        # self.label_Pre_Processing.setGeometry(540, 120, 500, 400) 
        # self.label_Pre_Processing.setAlignment(QtCore.Qt.AlignCenter)
        self.label_Pre_Processing.show()        
